...
def calculate_route():
    ...
    temperature = None if not float(data["temperature"]) else float(data["temperature"])
    coldIndex = None if not float(data["coldIndex"]) else float(data["coldIndex"])
    nIterations = None if not int(data["iteractions"]) else float(data["iteractions"])
    ...
    sa = SimulatedAnnealing(cityList, temperature = temperature, coldIndex = coldIndex, stoppingIteration = nIterations)
